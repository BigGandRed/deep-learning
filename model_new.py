...
random.seed(seed)
np.random.seed(seed)
# 移除GPU相关的随机种子设置，因为我们不使用GPU
# torch.cuda.manual_seed(seed) # 这行代码被移除了

# 设置设备信息
# 因为没有GPU，我们直接使用CPU作为设备
use_cuda = False # 修改为False，因为没有GPU
device = torch.device("cpu") # 直接设置为CPU设备
...
fold_num = 10
data_file = 'D:\新建文件夹4/train_set.csv'
import pandas as pd
...
# test
test_data_file = 'D:\新建文件夹4/test_a.csv'
...
# build word encoder
word2vec_path = 'D:\新建文件夹4/word2vec.txt'
dropout = 0.15
...
save_model = 'D:\jupyter\model/cnn.bin'
save_test = 'D:\jupyter\model/cnn.csv'
